...
all_opevals = 0
mode = 0
depthConvergeCount = 0
opConvergeCount = 0
convergeCount = 0
lastAvgDepth = 0.0
lastMax = lastMean = lastMin = 0
lastBest = lastOp = 0
populations = []
mlp_relu_decay_data = []
mlp_relu_decay_model = None
lstm_treelstm_attention_data = []
lstm_treelstm_attention_model = None
predict = False
stop = False
record = False
...
def getCandidates(tournament_size, num_tournaments=10, random_state=None):
...
def _parallel_evolve(n_programs, op_limit, parents, X, y, sample_weight, seeds, params):
    ...
    # Build programs
    programs = []
    global all_opevals, record
    dim = X.shape[1]

    better_count = 0
    for i in range(n_programs):

        if all_opevals >= op_limit:
            break
        ...
        if parents is None:
            ...
            if method < method_probs[0] and mode == 0:
                # crossover
                donor, donor_index = _tournament()
                program, removed, remains, crossover_op_cost = parent.crossover(donor.program,
                                                             random_state, op_limit)
                # print(f'crossover_op_cost: {crossover_op_cost}')
                ...
                if crossover_fitness < 1e-18:
                    record = True
                    all_opevals += crossover_op_cost
                if record != True:
                    all_opevals += crossover_op_cost
                if all_opevals >= op_limit:
                    break
            elif method < method_probs[0] and mode == 1: # OSDGP
                donor, donor_index = _tournament()
                donor_y_predicts = donor.y_predicts
                receiver_fitness = parent.raw_fitness(X, y, np.ones((n_samples,)))
                program, removed, remains, crossover_op_cost, operator_data, subtree_data, receiver_data, donor_data = parent.OSDXO(dim, predict, donor.program, donor_y_predicts, random_state, op_limit)
                genome = {'method': 'Crossover',
                          'parent_idx': parent_index,
                          'parent_nodes': removed,
                          'donor_idx': donor_index,
                ...
                crossover_fitness = check_program.raw_fitness(X, y, np.ones((n_samples,)))
                append_data = []
                if crossover_fitness > receiver_fitness: # receiver is better
                    append_data = operator_data + subtree_data + receiver_data
                elif crossover_fitness < receiver_fitness: # donor is better
                    better_count += 1
                    append_data = operator_data + subtree_data + donor_data
                else:  # Equal fitness: pick randomly
                    if random_state.rand() < 0.5:
                        append_data = operator_data + subtree_data + receiver_data
                    else:
                        append_data = operator_data + subtree_data + donor_data

                global mlp_relu_decay_data
                mlp_relu_decay_data.append(append_data)
                genome = {'method': 'Crossover',
                          'parent_idx': parent_index,
                          'parent_nodes': removed,
                          'donor_idx': donor_index,
                ...
                # subtree_mutation
                program, removed, _ , submut_op_cost = parent.subtree_mutation(random_state, op_limit)
                # print(f'submut_op_cost: {submut_op_cost}')
                ...
                if submut_fitness < 1e-18:
                    record = True
                    all_opevals += submut_op_cost
                if record != True:
                    all_opevals += submut_op_cost
                if all_opevals >= op_limit:
                    break
                ...
                # hoist_mutation
                program, removed, hoistmut_op_cost = parent.hoist_mutation(random_state, op_limit)
                # print(f'hoistmut_op_cost: {hoistmut_op_cost}')
                ...
                if hoistmut_fitness < 1e-18:
                    record = True
                    all_opevals += hoistmut_op_cost
                if record != True:
                    all_opevals += hoistmut_op_cost
                if all_opevals >= op_limit:
                    break
                ...
                # point_mutation
                program, mutated, ptmut_op_count = parent.point_mutation(random_state, op_limit)
                ...
                if ptmut_fitness < 1e-18:
                    record = True
                    all_opevals += ptmut_op_count
                if record != True:
                    all_opevals += ptmut_op_count
                if all_opevals >= op_limit:
                    break
        ...
        # Deal with the None Type genome when the opevals limit is reached
        ...
        if parents is None: # gen = 0
            ...
            all_opevals += op_count
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X, y, oob_sample_weight)

        programs.append(program)
    print(f'Benefical genetic ratio: {better_count/n_programs}')
    return programs
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.mode = mode
        self.operator_enc = operator_enc
        self.subtree_enc = subtree_enc
        self.treelstm_dim = treelstm_dim
        self.decay_rate = decay_rate
        self.population_size = population_size
        self.hall_of_fame = hall_of_fame
        self.n_components = n_components
        self.generations = generations
        self.cost_generations = 0
        self.trainfit = 0
        self.validfit = 0
        self.opevals = opevals
        self.final_opevals = 0
        self.evaluations = 0
        self.tournament_size = tournament_size
        self.stopping_criteria = stopping_criteria
        self.const_range = const_range
        self.max_length = max_length
        self.max_canddepth = max_canddepth
    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            print('    |{:^33}|{:^42}|'.format('Population Average',
                                               'Best Individual'))
            print('-' * 4 + ' ' + '-' * 33 + ' ' + '-' * 42 + ' ' + '-' * 20)
            line_format = '{:>4} {:>8} {:>8} {:>16} {:>8} {:>16} {:>16} {:>8} {:>10}'
            print(line_format.format('Gen', 'Depth', 'Length', 'Fitness', 'Length',
                                     'Fitness', 'OOB Fitness', 'OP', 'Time Left'))
            ...
            oob_fitness = 'N/A'
            line_format = '{:4d} {:8.2f} {:8.2f} {:16g} {:8d} {:16g} {:>16} {:>8} {:>10}'
            if self.max_samples < 1.0:
                oob_fitness = run_details['best_oob_fitness'][-1]
                line_format = '{:4d} {:8.2f} {:8.2f} {:16g} {:8d} {:16g} {:16g} {:>8} {:>10}'
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        global mode, max_length, max_canddepth, operator_enc, subtree_enc, treelstm_dim, decay_rate
        mode = self.mode
        max_length = self.max_length
        max_canddepth = self.max_canddepth
        operator_enc = self.operator_enc
        subtree_enc = self.subtree_enc
        treelstm_dim = self.treelstm_dim
        decay_rate = self.decay_rate

        if mode == 1: 
            print(f'Operator encoding: Integer Embedding')
            print(f'Subtree encoding: Tree-LSTM with Self-Attention')
        ...
        X_valid = X[int(X.shape[0]*(1-(self.validation))):]
        y_valid = y[int(y.shape[0]*(1-(self.validation))):]
        X = X[:int(X.shape[0]*(1-(self.validation)))]
        y = y[:int(y.shape[0]*(1-(self.validation)))]
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if gen == 0:
                parents = None
                CONVERGOPTIMES = int(self.opevals/X.shape[0]/self.population_size)
            else:
                parents = self._programs[gen - 1]
                global populations
                populations = parents
            
            # print(f"OP: {int(all_opevals)}")
            if all_opevals >= int(self.opevals) or stop:
                break
            ...
            self.final_opevals = all_opevals
            self.evaluations = (gen+1)*self.population_size
            self.cost_generations = gen

            global predict
            if mode == 1 and gen > 0:
                dimensionx = 1 + treelstm_dim
                global mlp_relu_decay_model, mlp_relu_decay_data
                from .mlp_relu_decay import MLPTrainer
                if mlp_relu_decay_model is None:
                    mlp_relu_decay_model = MLPTrainer(
                        input_size=dimensionx,
                        hidden_size=128,
                        output_size=treelstm_dim,
                        device="cuda",
                        gpu_id=0
                    )
                mlp_relu_decay_x = [row[:dimensionx] for row in mlp_relu_decay_data]
                mlp_relu_decay_y = [row[dimensionx:] for row in mlp_relu_decay_data]
                num_samples = len(mlp_relu_decay_data)
                print(f"MLPReLUDecayTrainX shape: ({num_samples}, {len(mlp_relu_decay_x[0])}), MLPReLUDecayTrainY shape: ({num_samples}, {len(mlp_relu_decay_y[0])})")
                mlp_relu_decay_model.train(mlp_relu_decay_x, mlp_relu_decay_y)
                predict = True
            ...
            for i in range(len(population)):
                depth.append(population[i]._depth())

            # Convergence
            global lastAvgDepth, lastMax, lastMean, lastMin, lastBest, lastOp
            global depthConvergeCount, convergeCount, opConvergeCount

            if (self.opevals != 100000000000000):
                if abs(np.mean(fitness) - best_program.raw_fitness_) < EPSILON: 
                    break
                if (abs(best_program.raw_fitness_ - lastBest) <= EPSILON):
                    convergeCount += 1
                else:
                    convergeCount = 0
                if (abs(all_opevals - lastOp) <= EPSILON):
                    opConvergeCount += 1
                else:
                    opConvergeCount = 0
                print(f'opConvergeCount: {opConvergeCount}, convergeCount: {convergeCount}')
                if opConvergeCount >= CONVERGOPTIMES or convergeCount > 300:
                    break

            self.trainfit = best_program.raw_fitness_
            ...
            self.run_details_['all_opevals'] = all_opevals
            generation_time = time() - start_time
            self.run_details_['generation_time'].append(generation_time)

            lastAvgDepth = np.mean(depth)
            lastMax = np.max(fitness)
            lastMean = np.mean(fitness)
            lastMin = np.min(fitness)
            lastBest = best_program.raw_fitness_
            lastOp = all_opevals
        ...
        if self.validation > 0:
            fitness_final = []
            for i in range(len(population)):
                valid_fitness = population[i].raw_fitness(X_valid, y_valid, np.ones((X_valid.shape[0],)))
                fitness_final.append(valid_fitness)
            self.validfit = min(fitness_final)
            if self._metric.greater_is_better:
                    best_program = population[np.argmax(fitness_final)]
            else:
                best_program = population[np.argmin(fitness_final)]
    ...
    def get_trainfit(self):
        return self.trainfit
    
    def get_validfit(self):
        return self.validfit

    def get_opevals(self):
        return self.final_opevals

    def get_evaluations(self):
        return self.evaluations

    def get_generations(self):
        return self.cost_generations
...
class SymbolicRegressor(RegressorMixin, BaseSymbolic):
    ...
    def __init__(self,
        ...
        super(SymbolicRegressor, self).__init__(
            ...
            mode=mode, 
            operator_enc=operator_enc, 
            subtree_enc=subtree_enc, 
            treelstm_dim=treelstm_dim, 
